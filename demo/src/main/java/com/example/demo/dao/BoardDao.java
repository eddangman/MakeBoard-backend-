...
public class BoardDao {
    ...
    //생정자 주입 스프링이 자동으로 HikariCp Bean을 주입한다.
...
@Transactional
    ...
    public int getTotalCount() {
        String sql ="select count(*) as total_Count from board";//무조건 한건의데이터가 나온다.
    ...
    public List<Board> getBoards(int page) {
        //start 는 0, 10 ,20 ,30 ,는 1페이지 2페이지 3페이지로
        int start =(page-1)*10;
        String sql="select b.user_id,b.board_id,b.title,b.regdate,b.view_cnt,u.name from board b, user u where b" +
                ".user_id=u.user_id order by board_id desc limit :start,10";
